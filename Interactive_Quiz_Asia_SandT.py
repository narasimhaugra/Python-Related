...
while True:
    ...
    img = cv2.flip(img,1)
    hands, imp = detector.findHands(img, flipType=False)
    ...
    if qNo < qTotal:
        ...
        img, _ = cvzone.putTextRect(img, "Thank You!!, Quiz Completed", [100, 300], 2, 2, offset=50, border=5)
        img, _ = cvzone.putTextRect(img, f'Your Score: {score}%', [800, 300], 2, 2, offset=50, border=5)

    # Draw Progress Bar
    progressBarValue = 150 + (950//qTotal)*qNo
    cv2.rectangle(img, (150,600), (progressBarValue, 650), (0,255,0), cv2.FILLED)
